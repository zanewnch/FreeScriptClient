...
watch(
  () => supermarketSaleStore.barChartData,
  (newData) => {
    ...
    */
    ...
    const avgGrossIncome = newData.data.map((item: { [x: string]: string }) =>
      parseFloat(item['avg_gross_income'])
    )
    const avgTotal = newData.data.map((item: { [x: string]: string }) =>
      parseFloat(item['avg_total'])
    )
...
<template>
  <div class="container flex justify-center items-center">
    <el-row class="elRow w-full h-full">
      ...
      <el-col :span="24" class="flex justify-center items-start" style="margin-top: 2rem">
        <span
          >This chart display the total sale amount and gross income for the different product line.
          <br />
          As you can see,there is no significant variation among the various product lines,
          indicating there isn't specific product preferred by customers.
        </span>
      </el-col>
    </el-row>
  </div>
